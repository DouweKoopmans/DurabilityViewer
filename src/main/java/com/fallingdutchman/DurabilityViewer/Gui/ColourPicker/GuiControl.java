...
public class GuiControl extends GuiButton
{
    ...
    public static void drawNativeLine(float x1, float y1, float x2, float y2, float width, int colour)
    {
        ...
        tessellator.getWorldRenderer().begin(GL_LINES, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().pos(x1, y1, 0).endVertex();
        tessellator.getWorldRenderer().pos(x2, y2, 0).endVertex();
        tessellator.draw();
    ...
    public static void drawArrow(int x1, int y1, int x2, int y2, int z, int width, int colour, boolean arrowHead, int arrowHeadSize)
    {
        ...
        // Draw the line
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_QUADS, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().pos(x1, y2, z).endVertex();
        tessellator.getWorldRenderer().pos(x2, y2, z).endVertex();
        tessellator.getWorldRenderer().pos(x2, y1, z).endVertex();
        tessellator.getWorldRenderer().pos(x1, y1, z).endVertex();
        tessellator.draw();

        // If an arrow then draw the arrow head
        if (arrowHead && arrowHeadSize > 0)
        {
            tessellator.getWorldRenderer().begin(GL_TRIANGLES, DefaultVertexFormats.POSITION);
            tessellator.getWorldRenderer().pos(x2, 0 - arrowHeadSize / 2, z).endVertex();
            tessellator.getWorldRenderer().pos(x2, arrowHeadSize / 2, z).endVertex();
            tessellator.getWorldRenderer().pos(length, 0, z).endVertex();
            tessellator.draw();
        }
    ...
    {
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        tessellator.getWorldRenderer().pos(x + height, y + width, this.zLevel).tex((float)(u) * texMapScale, (float)(v + height) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x + height, y, this.zLevel).tex((float)(u + width) * texMapScale, (float)(v + height) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x, y, this.zLevel).tex((float)(u + width) * texMapScale, (float)(v) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x, y + width, this.zLevel).tex((float)(u) * texMapScale, (float)(v) * texMapScale).endVertex();
        tessellator.draw();
    }
    ...
    protected void drawCrossHair(int x, int y, int size, int width, int colour)
    {
        ...
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_LINES, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().pos(x - size, y, 0).endVertex();
        tessellator.getWorldRenderer().pos(x + size, y, 0).endVertex();
        tessellator.draw();

        tessellator.getWorldRenderer().begin(GL_LINES, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().pos(x, y - size, 0).endVertex();
        tessellator.getWorldRenderer().pos(x, y + size, 0).endVertex();
        tessellator.draw();
    ...
    public void drawTexturedModalRect(int x, int y, int x2, int y2, int u, int v, int u2, int v2)
    {
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        tessellator.getWorldRenderer().pos(x, y2, this.zLevel).tex((float)(u) * texMapScale, (float)(v2) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x2, y2, this.zLevel).tex((float)(u2) * texMapScale, (float)(v2) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x2, y, this.zLevel).tex((float)(u2) * texMapScale, (float)(v) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x, y, this.zLevel).tex((float)(u) * texMapScale, (float)(v) * texMapScale).endVertex();
        tessellator.draw();
    }
    ...
    public void drawTexturedModalRectF(int x, int y, int x2, int y2, float u, float v, float u2, float v2)
    {
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        tessellator.getWorldRenderer().pos(x, y2, this.zLevel).tex(u, v2).endVertex();
        tessellator.getWorldRenderer().pos(x2, y2, this.zLevel).tex(u2, v2).endVertex();
        tessellator.getWorldRenderer().pos(x2, y, this.zLevel).tex(u2, v).endVertex();
        tessellator.getWorldRenderer().pos(x, y, this.zLevel).tex(u, v).endVertex();
        tessellator.draw();
    }
    ...
    public void drawTexturedModalRect(int x, int y, int u, int v, int width, int height, float texMapScale)
    {
        Tessellator tessellator = Tessellator.getInstance();
        tessellator.getWorldRenderer().begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        tessellator.getWorldRenderer().pos(x + 0, y + height, this.zLevel).tex((float)(u + 0) * texMapScale, (float)(v + height) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x + width, y + height, this.zLevel).tex((float)(u + width) * texMapScale, (float)(v + height) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x + width, y + 0, this.zLevel).tex((float)(u + width) * texMapScale, (float)(v + 0) * texMapScale).endVertex();
        tessellator.getWorldRenderer().pos(x + 0, y + 0, this.zLevel).tex((float)(u + 0) * texMapScale, (float)(v + 0) * texMapScale).endVertex();
        tessellator.draw();
    }
